...
def load_data():
    try:
        df = pd.read_excel("Airline_Dataset_Updated_Final.xlsx")
...
# 🔧 FIX: Handle sorting safely
country_options = sorted(df['Country Name'].dropna().unique())
country = st.sidebar.multiselect(
    "🌍 Country",
    options=country_options,
    default=country_options[:3]  # Show first 3 by default
)
...
month = st.sidebar.multiselect(
    ...
    default=month_options  # All months by default
)
...
with col2:
    ...
    <h2 style='color:#ff4444;'>{int(delayed_count):,}</h2>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    ...
    <h2 style='color:#2ca02c;'>{int(cancelled_count):,}</h2>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    if len(filtered_df) > 0 and len(filtered_df['Month'].dropna()) > 0:
...
with tab3:
    ...
    if 'Route' in filtered_df.columns and len(filtered_df) > 0:
    ...
    if 'Airport Name' in filtered_df.columns and len(filtered_df) > 0:
